...
class GGGTracker():
    ...
    async def enable_ggg_tracker(self, ctx):
        if self.check_owner(ctx):
            ...
            await self.bot.say("Your server has been successfully been added to the notification list, you may soon see a flood of embed cards as the server catcches up")
        else:
